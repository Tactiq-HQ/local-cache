...
Since this action no longer uses compression, cache files created on different operating systems (Windows, Linux, macOS) are compatible with each other. The cache files are simple tar archives without compression, ensuring consistent behavior across all platforms.
